...
#         print("hey  "*4 + str(index))
        ...
        try :
            ...
            # img = img.astype('short')
            img = img
            # img = Image.fromarray((255*img).numpy().astype(np.uint8).transpose(1, 2, 0))
            img = self.crop(TF.to_pil_image(img))
            img = self.transform(img)
...
if (num_images_to_show ==1 ):
    ...
    axarr[1].title.set_text('Encoded \n Image')
...
for i in range(num_images_to_show):
    if (num_images_to_show==1):
        ...
        axarr[0].imshow((valid_batch[i].cpu().detach().permute(1, 2, 0) *0.5) + 0.5)
        axarr[1].imshow(encode_image[i].cpu().detach().reshape([3,144,44]).permute(1, 2, 0) *0.5 + 0.5)

        axarr[2].imshow((reconstructed_img_8[i].cpu().detach().permute(1, 2, 0) *0.5) + 0.5)
    else:
        axarr[i,0].axis('off')
        axarr[i,1].axis('off')
        axarr[i,2].axis('off')
...
for i in range(num_images_to_show):
    ...
    plt.savefig("./SaveImageDemo/anh_goc_{}.png".format(i),bbox_inches='tight',pad_inches = 0)
    plt.close()

    plt.imshow(encode_image[i].cpu().detach().reshape([3,144,44]).permute(1, 2, 0) *0.5 + 0.5)
    plt.axis('off')

    plt.savefig("./SaveImageDemo/anh_nen_{}.png".format(i),bbox_inches='tight',pad_inches = 0)
    plt.close()
 
    plt.imshow((reconstructed_img_8[i].cpu().detach().permute(1, 2, 0) *0.5) + 0.5)
    plt.axis('off')

    plt.savefig("./SaveImageDemo/anh_giai_nen_{}.png".format(i),bbox_inches='tight',pad_inches = 0)
    plt.close()
